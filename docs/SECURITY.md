...
Security fixes are applied to the current OSS beta release line (the `1.9.x` series, currently `1.9.3`, installed via `pip install tokenpak`). Older pre-beta versions are not maintained.
