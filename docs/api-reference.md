...
An optional `recorded_usage` object reports the subset with complete provider
usage. It contains `requests_observed`, `requests_total`, `failed_requests`, and
`facts` with token subtotals and a provenance-labeled cost subtotal. These are
subset values: full-session totals, guard inputs and forecast eligibility retain
missing usage and failed requests. A partial subtotal cannot establish the full
session cost. Older responses may omit this additive object.
...
tokenpak savings --verify --json
```

With `--verify`, the `verification` object compares the existing byte estimator
with `cl100k_base` on a packaged fixture corpus. It reports both counts, absolute
and relative divergence, and the scope. It does not reconstruct stored prompts
or change savings calculations. Missing tokenizer dependencies or encoding
data produce an explicit unavailable result. See
...
`POST /tpk/v1/sessions/guard-snapshot` and
`POST /tpk/v1/sessions/workload-snapshot` accept an explicit `session_id` JSON
field. Both require loopback access and a configured `X-TPK-Key`; they reject
browser origins and caller-provided snapshots or clocks. Responses use
`Cache-Control: no-store`. Workload observations require optional durable
accounting; disabled accounting returns HTTP 503 with
`workload_accounting_unavailable`.

These reads expose bounded observations and preserve missing-evidence reasons.
They do not meter another request or grant spending or recommendation permission.
See [request workload observations](request-workload-observations.md),
