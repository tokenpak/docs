...
This guide is for developers who want to build the TokenPak v1.23.0 image and
...
- A checkout of the public TokenPak v1.23.0 tag.
...
Build the source checkout as `tokenpak:v1.23.0`:

```bash
docker build -t tokenpak:v1.23.0 .
```
...
docker run --rm -d --name tokenpak-proxy \
  ...
  tokenpak:v1.23.0 python -m tokenpak.proxy.server
```
...
The repository's v1.23.0 `docker-compose.yml` is not an out-of-the-box
...
v1.23.0 does not ship a separately verified Kubernetes or cloud-service
