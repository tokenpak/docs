...
This guide is for developers who want to build the TokenPak v1.25.3 image and
...
- A checkout of the public TokenPak v1.25.3 tag.
...
Build the source checkout as `tokenpak:v1.25.3`:

```bash
docker build -t tokenpak:v1.25.3 .
```
...
docker run --rm -d --name tokenpak-proxy \
  ...
  tokenpak:v1.25.3 python -m tokenpak.proxy.server
```
...
The repository's v1.25.3 `docker-compose.yml` is not an out-of-the-box
...
v1.25.3 does not ship a separately verified Kubernetes or cloud-service
